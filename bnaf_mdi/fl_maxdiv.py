...
def denseRegionProposals(N, int_min=10, int_max=100):
    ...
    int_sizes = [int_min]
    if (int_min < int_max):
        #int_sizes = np.unique(np.logspace(np.log10(int_min), np.log10(int_max), 100).astype(int))
...
if (__name__ == "__main__"):
    ...
    data = np.load('../soft_hidden/x_anomaly_11.npy')
    data = np.load('../soft_hidden/h.npy')
    data_attack = np.load('../soft_hidden/smoother.npy')
